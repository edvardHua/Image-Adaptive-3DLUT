...
LUTs = torch.load("%s/LUTs.pth" % opt.model_dir, map_location="cpu")
...
# LUT3.eval()
# LUT4.eval()
classifier.load_state_dict(torch.load("%s/classifier.pth" % opt.model_dir, map_location="cpu"))
classifier.eval()
...
if opt.input_color_space == 'sRGB':
    ori_img = Image.open(opt.image_path)
    ori_img = ori_img.convert("RGB")
    img = TF.to_tensor(ori_img).type(Tensor)
...
fn = os.path.basename(opt.image_path)
