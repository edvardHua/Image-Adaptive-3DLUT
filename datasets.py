...
def convert_tif_to_jpg():
    # 转换 ppr 10k 的 tif 为 jpg 格式的图片
    input_path = "/Users/zihua.zeng/Dataset/色彩增强数据集/ppr_10k/source"
    target_path = "/Users/zihua.zeng/Dataset/色彩增强数据集/ppr_10k/target_b"

    for fn in os.listdir(target_path):
        if not fn.endswith(".tif"):
            continue
        img = cv2.imread(os.path.join(target_path, fn))
        cv2.imwrite("%s/%s" % (target_path, fn.replace(".tif", ".jpg")), img)
...
class ImageDataset_PPR10K_sRGB(Dataset):
    def __init__(self, root, mode="train"):
        ...
        self.set1_expert_files = os.listdir(os.path.join(root, "target_b"))
        self.set1_input_files.sort()
        self.set1_expert_files.sort()

        if self.mode == "train":
            self.prefix = "source"
            self.set1_input_files = self.set1_input_files[:-500]
            self.set1_expert_files = self.set1_expert_files[:-500]

        if self.mode == "test":
            self.prefix = "target_b"
            self.set1_input_files = self.set1_input_files[-500:]
            self.set1_expert_files = self.set1_expert_files[-500:]
    ...
    def __getitem__(self, index):
        ...
        img_name = self.set1_input_files[index_id]
        img_input = Image.open(os.path.join(self.root, self.prefix, self.set1_input_files[index_id]))
        img_exptC = Image.open(os.path.join(self.root, self.prefix, self.set1_expert_files[index_id]))
