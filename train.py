...
parser.add_argument("--model_type", type=str, default="tpami", help="tpami, ailut")
...
if not opt.output_dir:
    opt.output_dir = time.strftime("%m-%d_%H_%M_%S") + "_%s" % opt.model_type
...
LUT0 = Generator3DLUT_identity(dim=opt.n_vertices)
LUT1 = Generator3DLUT_zero(dim=opt.n_vertices)
LUT2 = Generator3DLUT_zero(dim=opt.n_vertices)

model = None
if opt.model_type == "tpami":
    model = TPAMIBackbone()
    model.apply(weights_init_normal_classifier)
elif opt.model_type == "ailut":
    pass
...
optimizer_G = torch.optim.Adam(
    itertools.chain(model.parameters(), LUT0.parameters(), LUT1.parameters(), LUT2.parameters()),
    lr=opt.lr,
    betas=(opt.b1, opt.b2))
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if avg_psnr > max_psnr:
        ...
        torch.save(model.state_dict(), "saved_models/%s/best_model/classifier_%d.pth" % (opt.output_dir, epoch))
