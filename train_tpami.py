...
if not opt.output_dir:
    opt.output_dir = time.strftime("%m-%d_%H_%M_%S") + "_tpami"
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if avg_psnr > max_psnr:
        ...
        torch.save(model.state_dict(), "saved_models/%s/best_model/tpami_%d.pth" % (opt.output_dir, epoch))
    ...
    if epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(model.state_dict(), "saved_models/%s/tpami_%d.pth" % (opt.output_dir, epoch))
